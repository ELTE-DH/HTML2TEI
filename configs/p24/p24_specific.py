...
def get_meta_from_articles_spec(tei_logger, url, bs):
    ...
    section_match = url.split('/')  # [-6]

    if section_match[-6] in SUBJECT_DICT.keys():
        data['sch:articleSection'] = SUBJECT_DICT[section_match[-6]]
    elif section_match[-7] in SUBJECT_DICT.keys():
        data['sch:articleSection'] = SUBJECT_DICT[section_match[-7]]
    else:
        tei_logger.log('WARNING', f'{url}: ARTICLE SECTION {section_match} NOT FOUND!')
