...
def get_meta_from_articles_spec(tei_logger, url, bs):
    ...
    data['sch:url'] = url
    write_it = ''
    data = json.loads(bs)   # TODO: a get metánál megoldani, hogy ez json
    title = data['title']
    pub_date = data['public_date']
    if 'modify_date' in data.keys():    # 2020.10.13. 12:37
        modified_date = data['modify_date']
    column = [rov['cim'] for rov in data['rovat']]
    authors = [rov['nev'] for rov in data['authors']]
    keywords = [rov['cim'] for rov in data['tags']]
